...
\section{Introduction}
 ...
 This project used a pole on a cart (a.k.a. Cart-Pole) model as an inverted pendulum system and investigated the use of different estimators: the Kalman Filter (KF), the Extended Kalman Filter, and the Unscented Kalman Filter.  Since the KF used a linearized model of the system, it is hypothesized that its performance will degrade at larger angles as the small angle approximation will be used to linearize the equations of motion.  The project also tested the use of a single sensor and two sensors, and it is expected that two sensors will yield a more accurate estimation of the states.  Finally, this project evaluated the RMSE (root mean square error) and run time across estimators (KF, EKF, UKF) with different tuning.  The following sections discuss the system, state space models, test cases, results, and conclusions.
...
\begin{figure}[h!]
 ...
 \end{figure}
Using the above assumptions, the non-linear equations of motion\footnote{The equations have no moment of inertia term because assumption (3) results in $J=\frac{1}{12}m(2l)^2$.} were derived using Newton's second law (Equation \ref{eq:EOM_nonlinear}).  The description of the parameters and their values are given in Table \ref{table:paramvalues}.
\begin{equation}
\begin{aligned}
...
The Kalman Filter (KF) is the best linear estimator for nonlinear systems, and in order to implement it for a nonlinear system, the equations must be linearized. The control objective is to maintain the center of mass directly over the pivot point, which corresponds to a vertical position defined as $\theta$ = 0$^\circ$. Therefore, this is the angle about which to linearize; this is often called a "small angle approximation." This first-order truncation of the trigonometric Taylor series expansions results in the following substitutions to the nonlinear equations:
\begin{align*}
...
\begin{equation*}
\begin{aligned}
\matr{\dot{x}} &= \matr{A}\matr{x} + \matr{B}\matr{u} \\
...
\end{equation}
\subsubsection{Stability, Controllability, Observability, and Detectability Analysis}
...
The purpose of the simulator is to generate ground truth (i.e. actual states), input, and measurement data for use in offline test cases, as well as for online comparison of estimators.  For the offline test cases, all the estimators recieve the same input and allows a more direct comparison. The simulator used in this project is based on the general system architecture (See Figure \ref{fig:block_diagram}) and contains modifications (See Appendix \ref{simdetails}).  The simulation is modeled from a simulator in OpenAI Gym \cite{OpenAI}. The parameters of the system and input constraints were modified to reflect an actual inverted pendulum system \cite{lab6}. Process noise and measurement noise are also included based on the analysis discussed in Section \ref{ssecsensornoise}.  The animation produced by the simulator is shown in Figure \ref{fig:simulator}.
\begin{figure}
	\centering
...
Several online test cases were conducted to determine the largest starting pole angle across the estimators\footnote{The output is limited to represent force saturation, which restricts the ability of the controller to accomodate all possible starting pole angles.}. Table \ref{table:onlinecases} summarizes the results and shows that two sensors enable larger starting pole angles than a single sensor.  The nonlinear estimators can accomodate larger staring pole angles than the KF.  In the cases with a single sensor, none of the estimators coudl provide sufficiently accurate state estimation to the controller with the selected sensor.  It was possible to make a single sensor feasible by increasing the sensor accuracy.
\begin{table}[h!]
	\centering
	\renewcommand{\arraystretch}{1.5}
	\begin{tabular}{ |l |c  |c |c | c|}
		...
		& $\sigma = 0.02$ & $\sigma=0.004$ & $\hat{Q} = Q$ & $\hat{Q} = 0.001Q$ \\ \hline
...
\end{bmatrix}
\end{equation}
A quick check with MATLAB using the command rank($\mathcal{C}$) tells us $\mathcal{C}$ is rank(4), which by the textbook definition makes this system controllable \cite{simon}. The initial attempt at using a Kalman filter with just one measurement (cart position) was not working very well. It was then determined to check the observability matrix $\mathcal{O}$, defined as:
\begin{equation}
...
\end{bmatrix}
\end{equation}
This matrix was also full rank, which was perplexing. Professor Lin suggested checking the condition number of the observability matrix. Using the MATLAB command cond(), the result was cond($\mathcal{O}$) = $6.67e+5$, which suggests that the observability matrix is almost singular. This was the source of some of the problems in implementing the state estimators with a single sensor. Based on this information, another sensor was added for the pole angular rate. After doing this, the condition number of the observability matrix was $60.437$, a much more reasonable number.
\subsection{Details of Process Noise}\label{appendix:processNoise}
Process noise was injected to the system through the input force. A motor model was constructed with constant, linear coefficients of system variables. From this model,the input force was assumed to follow a linear relation with controlling voltage of motor circuit shown as Figure \ref{fig:circuit}.
\begin{figure}[h!]
	\centering
	\includegraphics[width=10cm,keepaspectratio]{circuit.png}
	\caption{Motor Circuit Diagram}
	\label{fig:circuit}
\end{figure}
The linear relationship between force noise and motor voltage input noise\footnote{A value for $\alpha$ was selected based on small motors comparable to what would be required for a small cart-pole robot. A lab acted as a guide and assisted with construction of a motor model \cite{lab3}.} is expressed as Equation \ref{FvsV}, $\alpha \approx 1.72$ (Value from \cite{lab3}). It was assumed that the motor driver operates at 12 Volts and with a 10 bit DAC accuracy. By this analysis, the standard deviation $\sigma$ of the input noise can be approximated as $\alpha\frac{12}{2^{10}}$ = 0.02.
\begin{equation}
F = \alpha V + B
\label{FvsV}
\end{equation}
\subsection{Details of the Controller}
In simulation, a discretized Linear Quadratic Regulator was used to output desired force at each time step. The idea is to use dynamic programming methodology to perform backward recursion calculations starting from the final desired position. At each step, using the linear constraint and quadratic objective function, the desired force can be calculated by a gradient method. Then the controller will choose first step of calculated force as input of system at current time step. The calculation for loop is shown in Figure \ref{fig:iLQR}.
\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm,keepaspectratio]{LQR.png}
	\caption{Iterative Linear Quadratic Regulator (iLQR) (From \cite{puterman2014markov})}
	\label{fig:iLQR}
\end{figure}
\subsection{Discussion of Simulation Environment, Online versus Offline}\label{simdetails}
The method of simulation can be ascertained by a study of Figure \ref{fig:simulation}. The offline ground truth data was generated by inserting parameters and initial conditions to the Python OpenAI Gym package \cite{OpenAI}. Then, this data was loaded into MATLAB for offline estimator performance comparison. 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=17cm,keepaspectratio]{Simulation.png}
