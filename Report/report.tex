...
y = \begin{bmatrix}
1 & 0 & 0 & 0 \\
>>>>>>> 9393d4d75ad9e14d2446a3eafd3074e7d6df4c02
\end{bmatrix} \begin{bmatrix}
	x \\
	\dot{x} \\
	\theta \\
...
\\
The Extended Kalman Filter differs from the regular Kalman Filter by utilizing the nonlinear discrete time state equations in place of the linear equations. This is accomplished by inserting the previous \textit{a posteriori} estimates of the states into the discrete nonlinear equations to get the \textit{a priori} estimate of the states at the current time step. This necessitates that the the nonlinear discrete time equations be computed. This is accomplished using a first-order Euler approximation of the continuous time versions (Equation \ref{eq:nonlinear_ss_continuous}):
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Next, the Jacobian of the above equation will be evaluated at each time step for the \textit{a priori} \textit{P} covariance matrix update:
\begin{align}
\begin{aligned}
...
\end{aligned}
\end{align}
Then $\matr{P}_{k|k-1}$ is computed as
\begin{equation}
